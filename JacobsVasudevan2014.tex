...
\hoj{Rather than bad-mouthing Koopman/Perron-Frobenius operators,
consider speaking positively about the need for spectral theories
and indicate a gap in the literature which this paper fills.
The ``benefits'' can be mentioned briefly here and elaborated later.}
...
The constraint ``$\phi^*W_s \subset i_U^* C^k(M)$'' enforces that each of the spaces $W_s$ models the function space $i_U^* C^k(M)$ and nothing more.
...
\begin{example}
Let $W_N$ denote the space of sinuisoids $\sin( k x),\cos( kx)$ for $k < N$ restricted to the interval $(-\pi,\pi)$.
...
which can be extended to $C^k$ functions on $[-\pi,\pi]$ with periodic boundary conditions.
From here it is easy to observe that $\phi$ is a computational domain with respect to $W_N$ for any integer $N$.
\end{example}
